...
public class QuartzScheduler {
     ...
     */
    public void startJob(String cronValue) throws SchedulerException {
        startJob1(scheduler,cronValue);
//        startJob2(scheduler);
        scheduler.start();
    }
    ...
    private void startJob1(Scheduler scheduler,String cronValue) throws SchedulerException {
        // 通过JobBuilder构建JobDetail实例，JobDetail规定只能是实现Job接口的实例
        // JobDetail 是具体Job实例
        JobDetail jobDetail = JobBuilder.newJob(SchedulerQuartzJob1.class).withIdentity("job1", "group1").build();
        // 基于表达式构建触发器
        // CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule("0/5 * * * * ?");
        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule(cronValue);
